// Phase 1 of the UX revamp: the welcome screen no longer asks for
// team / city / project-type and a "RATE 133 METHODS" promise. It
// asks one focused question — "what are you working on?" — and
// answers it with a concrete shortlist of methods generated by AI.
// Sign-in, browsing, and dashboards are all one tap away but no
// longer in front of the user before they see value.
...
export function WelcomeView() {
  ...
  const canSubmit = desc.trim().length >= 20 && hasMistral && !busy
  ...
  const submit = async (e) => {
    ...
    setBusy(true); setErr('')
    try {
      const out = await suggestMethods({
        name: name.trim() || 'Your project',
        desc: desc.trim(),
      })
      if (!out.length) throw new Error('No matching methods returned. Try rephrasing.')
      setProjectContext({ name: name.trim() || 'Your project', desc: desc.trim() })
      ...
      <div style={{
        ...
        <form onSubmit={submit}
          ...
          <ScrappyButton type="submit"
            ...
            size="lg" full>
            {busy ? 'ANALYSING…' : '✨ GET MY METHOD SHORTLIST →'}
          </ScrappyButton>
        </form>
