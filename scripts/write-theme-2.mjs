...
const GATE_VOLT_ARR = "['','#C17B2A','#1B5FA0','#2A6B45','#7A3A8E']"
const GATE_RGBA = `function gateRgba(g, a) {
...
import {
  GATE_DARK, GATE_LABEL, TOOLS,
  toolsForGate, practicedForGate, FAMILY_STYLE, SCOPE_STYLE,
} from '../data/tools'

const GATE_VOLT = ${GATE_VOLT_ARR}
...
function speak(tool) {
  ...
  const text = [tool.n + '.', tool.def, 'Conseil praticien :', tool.t].join(' ')
...
function GateComplete({ gate }) {
  ...
  return (
    <div className="anim-fadein" style={{ textAlign: 'center', padding: '40px 16px' }}>
      <div style={{ fontSize: '52px', marginBottom: '8px' }} className="anim-pop">⭐</div>
      <div className="text-mega" style={{ fontSize: 'clamp(40px,12vw,64px)', color: col, marginBottom: '4px' }}>
        GATE {gate}
      </div>
      <div className="text-mega" style={{ fontSize: '22px', color: '#1C2530', marginBottom: '20px' }}>
        EXPLORÉ
      </div>
      <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px',
        padding: '10px 20px', borderRadius: '12px', marginBottom: '28px',
        background: gateRgba(gate, .08), border: '1px solid ' + gateRgba(gate, .25) }}>
        <span className="text-mega" style={{ fontSize: '28px', color: col }}>{pr}/{tools.length}</span>
        <span style={{ fontSize: '12px', color: '#8B8074', fontWeight: 700 }}>méthodes pratiquées</span>
      </div>
      <br />
      <button onClick={goMap}
        style={{ padding: '14px 32px', borderRadius: '12px', cursor: 'pointer',
...
function FlipCard({ tool, gate, onFlip, flipped }) {
  ...
  const col   = GATE_VOLT[gate]
  const dark  = GATE_DARK[gate]  // front reste sombre — contraste sur fond parchemin
  ...
  return (
    <div className="perspective-900" style={{ width: '280px', height: '390px' }}>
      ...
      >
        {/* Front — sombre (face non révélée) */}
        <div className="absolute inset-0 backface-hidden overflow-hidden"
          style={{ borderRadius: '20px', background: dark,
            border: '1px solid rgba(255,255,255,.06)',
            display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
            boxShadow: '0 8px 32px rgba(28,37,48,.18), 0 2px 8px rgba(28,37,48,.12)' }}>
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: col, borderRadius: '20px 20px 0 0' }} />
          ...
          <div style={{ position: 'absolute', bottom: '20px', textAlign: 'center' }}>
            <div style={{ fontSize: '10px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.08em',
              color: 'rgba(255,255,255,.3)' }}>Toucher pour révéler</div>
            <div style={{ fontSize: '16px', marginTop: '4px', color: col }}>↓</div>
          </div>
        </div>

        {/* Back — clair (fiche méthode) */}
        <div className="absolute inset-0 backface-hidden rotate-y-180 overflow-hidden flex flex-col"
          style={{ borderRadius: '20px', background: '#FFFDF8',
            boxShadow: '0 8px 32px rgba(28,37,48,.12)' }}>
          <div style={{ height: '4px', flexShrink: 0, background: col, borderRadius: '20px 20px 0 0' }} />
          <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
            ...
            {/* Def */}
            <p style={{ fontSize: '12px', color: '#6B6460', lineHeight: 1.55, marginBottom: '10px', flexShrink: 0 }}>
              {tool.def}
            </p>
            {/* Gate usages */}
            <div style={{ fontSize: '10px', fontWeight: 800, textTransform: 'uppercase',
              letterSpacing: '.05em', color: '#B0A898', marginBottom: '6px' }}>Usage par gate</div>
            <div style={{ overflowY: 'auto', flex: 1 }}>
              {tool.g.filter(g => tool.gu[g]).map(g => (
                <div key={g} style={{ display: 'flex', gap: '6px', alignItems: 'flex-start', marginBottom: '4px' }}>
                  ...
                  <p style={{ fontSize: '11px', color: '#6B6460', lineHeight: 1.4 }}>{tool.gu[g]}</p>
                </div>
              ))}
            </div>
          </div>
          {/* Tip */}
          <div style={{ background: '#EFF7F0', borderTop: '1px solid #C3E6C9',
            ...
            <p style={{ fontSize: '11px', color: '#2A6B45', lineHeight: 1.45 }}>{tool.t}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px',
      ...
      <button onClick={() => onAction('practice')}
        style={{ padding: '14px', borderRadius: '12px', border: 'none', cursor: pr ? 'default' : 'pointer',
          background: pr ? '#C3E6C9' : '#2A6B45', color: pr ? '#2A6B45' : '#fff',
      ...
      <button onClick={() => onAction('flag')}
        style={{ padding: '14px', borderRadius: '12px', border: 'none', cursor: 'pointer',
          background: fl ? '#D8C8EA' : '#1B3D6F', color: fl ? '#7A3A8E' : '#fff',
          fontFamily: 'Barlow Condensed, sans-serif', fontSize: '16px', fontWeight: 900 }}>
      ...
      <button onClick={() => onAction('skip')}
        style={{ padding: '14px', borderRadius: '12px', cursor: 'pointer',
          background: '#EAE5DB', color: '#8B8074',
    ...
    <div style={{ display: 'flex', gap: '5px' }}>
      {tools.map((_,i) => (
        <div key={i} style={{
          ...
          background: i < idx ? '#CFC9BE' : i === idx ? '#1B3D6F' : '#E0DAD2',
        }} />
      ))}
    </div>
  )
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
        <button onClick={() => { window.speechSynthesis?.cancel(); goMap() }}
          style={{ padding: '6px 12px', borderRadius: '10px', cursor: 'pointer',
            ...
            color: '#8B8074', fontSize: '12px', fontWeight: 800 }}>
          ← CARTE
        </button>
        <div style={{ flex: 1 }}>
