...
lines.push(']')
lines.push('')
lines.push('// BASE prefix — Vite injects "/UrbanQuest/" on GitHub Pages and "/"')
lines.push('// in local dev. Without this the assets in /public/canvas/ resolve')
lines.push('// to the wrong host root and 404 in production.')
lines.push('const BASE = (import.meta.env?.BASE_URL ?? "/").replace(/\\/+$/, "/") || "/"')
lines.push('')
lines.push('export function canvasUrl(toolId) {')
lines.push('  const file = CANVAS_FILES[toolId]')
lines.push('  return file ? `${BASE}canvas/${file}` : null')
lines.push('}')
lines.push('')
lines.push('export function canvasThumbUrl(toolId) {')
lines.push('  const id = String(toolId).padStart(3, "0")')
lines.push('  return `${BASE}canvas-thumbs/OVPM-${id}.png`')
lines.push('}')
lines.push('')
